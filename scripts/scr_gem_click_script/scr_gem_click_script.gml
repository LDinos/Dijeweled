...
///@param change_xy
///@param Gamerule
function scr_gem_click_script(argument0, argument1, argument2, argument3, argument4, argument5, argument6) {
	var GM = argument0
	...
	if !GM.amLocked //&& !GM.ammoving //if gem that we want to check is not locked or moving due to match
			{	
				var domatch = false;
					...
					else if gem1 = MyGamerule.gems_id_fallen_array[@ylim,xlim] //if we have a gem1 and that gem1 is the gem we just selected
					{
							...
							{						
								...
								#region horizontal
									if (xlim - xlimprevious < 0) //we swapped left
									{
										if xlimprevious != 7
										{
											if (MyGamerule.gems_id_fallen_array[ylim,xlim+2] != noone)
											{
								...
								#region vertical
									if (ylim - ylimprevious < 0) //we swapped up
									{
										if ylimprevious != 7
										{
											if (MyGamerule.gems_id_fallen_array[ylim+2,xlim] != noone)
											{
								...
								if MyGamerule.gems_id_fallen_array[ylim,xlim].amLocked canmove = false 
							}
						#endregion
							if canmove && multiswap_allowed
							{
								gem2 = MyGamerule.gems_id_fallen_array[@ylim,xlim]
					...
					else //if gem1 exists, and we select a gem far away
						{
							if !GM.amInvisible
							{
								...
								mouse_yy_pos = argument2
								with(gem1) unspin()
								gem1 = MyGamerule.gems_id_fallen_array[@ylim,xlim] //make new gem1
								with(gem1) spin()
							}
							//else gem1 = noone
							GM = noone
