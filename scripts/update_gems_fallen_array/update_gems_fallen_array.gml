...
function update_gems_fallen_array() {
	...
	for(var i = 0; i < 8; i++) for(var j = 0; j < 8; j++) {
		temp_array[i][j] = gems_id_array[i][j]
	...
	for(var j=0;j<=7;j++)
	{
		...
		for(var i=7;i>=0;i--)
		{
			if temp_array[i][j] != noone
			{
				...
				gems_id_array[k][j] = temp_array[i][j]
				gems_id_array[k][j].i_limit = k
...
function update_gems_to_spawn_array(){
	for(var j = 0; j < 8; j++) for(var i = 0; i < 8; i++) {
		if (gems_skin_array[i][j] == -1) gems_to_spawn[i][j] = irandom(num_skin)
		else break
	}
}